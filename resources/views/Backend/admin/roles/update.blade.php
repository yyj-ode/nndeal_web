...
@section('content')
    <div style="margin: 15px;">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="display_name" lay-verify="required" name="display_name" autocomplete="off" value="{{$result->display_name}}" placeholder="请输用户名称" class="layui-input" value="">
            </div>
        </div>
            ...
            <div class="layui-input-inline">
                <select name="cat_id" class="one select" lay-filter="one">
                    @foreach($data_menu as $k=>$v)
                        <option value="{{$v['id']}}" @if($v['id'] == $result->cat_id)selected="selected"@endif>{{$v['html']}}{{$v['name']}}</option>
                    @endforeach
                </select>
            </div>
        </div>
    ...
    <script type="text/javascript">
        layui.use(['form', 'layedit', 'laydate'], function() {
            ...
            form.on('submit(submit)', function(data) {
                ...
                loadShow();
                $.ajax({
                    url: "{{url('admin/roles/update_save')}}",
                    type: 'post',
                    data: data.field,
                            ...
                            layer.open({
                                ...
                                success: function(layero){
                                    ...
                                    btn.find('.layui-layer-btn1').attr({
                                        href: "{{url('admin/roles/update/?id='.$result->id)}}",
                                        target: '_self'
                                    });
                                    layer.close(layero);
                                }
