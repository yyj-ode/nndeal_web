    ...
    <div style="margin: 15px;">
        ...
        <div class="layui-form-item" id="city_china">
            ...
            <div class="layui-input-inline">
                <select name="users_id" class="one select" lay-filter="one" lay-verify="required" >
                    <option value="">请选择</option>
                    @foreach($users as $key=>$value)
                        <option <?php if($result['users_id'] == $value->id)
                            {
            ...
            <div class="layui-input-inline">
                <select name="shop_id" class="one select" lay-filter="one" lay-verify="required" >
                    <option value="">请选择</option>
                    @foreach($shop as $key=>$value)
                        <option <?php if($result['shop_id'] == $value->id)
                        {
                            echo "selected = 'selected'";
                        }
                                ?> value="{{$value->id}}">{{$value->shop_name}}</option>
                    @endforeach
                </select>
            </div>
        </div>
                            ...
                            layer.open({
                                ...
                                success: function(layero){
                                    ...
                                    btn.find('.layui-layer-btn0').attr({
                                        href: '{{url('admin/collection')}}',
                                        target: '_self'
                                    });
                                    btn.find('.layui-layer-btn1').attr({
                                        href: '{{url("admin/collection/update?id=$result[id]")}}',
                                        target: '_self'
                                    });
                                    layer.close(layero);
                                }
