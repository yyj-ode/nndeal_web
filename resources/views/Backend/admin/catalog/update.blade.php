...
@section('content')
    <div style="margin: 15px;">
        <blockquote class="layui-elem-quote">
            <a class="layui-btn layui-btn-small" href="{{url('admin/catalog/index')}}"><cite>全部分类</cite></a>
        ...
        <div class="layui-form-item" id="city_china">
            ...
            <div class="layui-input-inline">
                <select name="parent_id" class="one select" lay-filter="one">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="icon" lay-verify="required" autocomplete="off" placeholder="请输用户名称" class="layui-input" value="{{$result->icon}}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">路由</label>
            <div class="layui-input-inline">
                <input type="text" name="model"  value="{{$result->model}}" lay-verify="required" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">排序</label>
            <div class="layui-input-inline">
                <input type="text" name="sort" lay-verify="int"  value="{{$result->sort}}" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">状态</label>
            <div class="layui-input-block">
                <input type="radio" name="status"  @if($result->status == 1) checked="checked" @endif value="1" title="开启" >
                <input type="radio" name="status" value="0" @if($result->status == 0) checked="checked" @endif title="关闭" >
            </div>
        </div>
    ...
    <script type="text/javascript">
        layui.use(['form', 'layedit', 'laydate'], function() {
            ...
            form.on('submit(submit)', function(data) {
                ...
                loadShow();
                $.ajax({
                    url: "{{url('admin/catalog/update_save')}}",
                    type: 'post',
                    data: data.field,
                    headers: { 'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content') },
                    success:function(result){
                            ...
                            layer.open({
                                ...
                                success: function(layero){
                                    ...
                                    btn.find('.layui-layer-btn0').attr({
                                        href: '{{url('admin/catalog/index')}}',
                                        target: '_self'
                                    });
                                    btn.find('.layui-layer-btn1').attr({
                                        href: "{{url('admin/catalog/update/?id='.$result->id)}}",
                                        target: '_self'
                                    });
                                    layer.close(layero);
                                }
