...
@section('content')
    ...
    <blockquote class="layui-elem-quote">
        <a class="layui-btn layui-btn-small" href="{{url('admin/catalog/index')}}"><cite>全部类别</cite></a>

            <a href="{{url('admin/catalog/create')}}" class="layui-btn layui-btn-small" id="add">
        ...
        layui.use(['jquery', 'form', 'layedit', 'laydate'], function () {
            $(function () {
                var contents = $("#contents").DataTable({
                    ...
                    order: [[0, "desc"]],
                    ajax: {
                        url: "{{url('admin/catalog/indexdata')}}",
                        type: "POST",
                        headers: {
                    ...
                    columnDefs: [

                        {
                            ...
                            render: function (data, type, row, meta) {
                                return "<a style='float: left;' href='/admin/catalog/update?id=" + row.id + "' >"+ row.html + row.name + "</a>"
                            }
                        },
                        {
                            'targets': -1, "render": function (data, type, row) {
                        var html = '<a style="margin:3px;" href="/admin/catalog/update?id=' + row.id + '" class="X-Small btn-xs text-danger" title="修改"><i class="linyer icon-edit"></i></a>';
                            html += '<a style="margin:3px;" href="/admin/catalog/create" title="添加"><i class="linyer icon-xgpwd2"></i></a>';
            ...
            layer.open({
                ...
                ,success: function(layero){
                    ...
                    btn.find('.layui-layer-btn0').attr({
                        href: '{{url('admin/catalog/del/?id=')}}'+id,
                    });
                }
            });
        }
